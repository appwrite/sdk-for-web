...
class BrowserContext extends _instrumentation.SdkObject {
  constructor(browser, options, browserContextId) {
    ...
    if (this._options.recordHar) this._harTracer = new _harTracer.HarTracer(this, this._options.recordHar);
  ...
  async _initialize() {
    ...
    if ((0, _utils.debugMode)() === 'console') await this.extendInjectedScript('main', consoleApiSource.source);
  }
  ...
  async exposeBinding(name, needsHandle, playwrightBinding, world) {
    const identifier = _page.PageBinding.identifier(name, world);

    if (this._pageBindings.has(identifier)) throw new Error(`Function "${name}" has been already registered`);

    for (const page of this.pages()) {
      if (page.getBinding(name, world)) throw new Error(`Function "${name}" has been already registered in one of the pages`);
    }

    const binding = new _page.PageBinding(name, playwrightBinding, needsHandle, world);
  ...
  async close(metadata) {
    if (this._closedStatus === 'open') {
      var _this$_harTracer;

      this.emit(BrowserContext.Events.BeforeClose);
      this._closedStatus = 'closing';
      await ((_this$_harTracer = this._harTracer) === null || _this$_harTracer === void 0 ? void 0 : _this$_harTracer.flush());
