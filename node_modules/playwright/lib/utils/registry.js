...
function readDescriptors(packagePath) {
  const browsersJSON = require(_path.default.join(packagePath, 'browsers.json'));
...
class Registry {
  ...
  _addRequirementsAndDedupe(executables) {
    const set = new Set();
    if (!executables) executables = this._executables.filter(executable => executable.installType === 'download-by-default');
  ...
  async _installChromiumChannel(channel, scripts, scriptArgs = []) {
    ...
    const {
      code
    } = await (0, _utils.spawnAsync)(shell, [_path.default.join(BIN_PATH, scriptName), ...scriptArgs], {
      cwd: BIN_PATH,
      stdio: 'inherit'
    });
...
const registry = new Registry(PACKAGE_PATH);
exports.registry = registry;