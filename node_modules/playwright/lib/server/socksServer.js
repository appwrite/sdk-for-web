...
const BUF_REP_INTR_SUCCESS = Buffer.from([0x05, SOCKS_REPLY.SUCCESS, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00]);
/**
...
class SocksV5ServerParser {
  constructor(socket) {
    ...
    this._socket = void 0;
    this._parsingFinishedResolve = void 0;
    this._parsingFinishedReject = void 0;
    this._parsingFinished = void 0;
    this._info = void 0;
    this._phase = ConnectionPhases.VERSION;
    this._authMethods = void 0;
    this._dstAddr = void 0;
    this._addressType = void 0;
    this._methodsp = 0;
    this._socket = socket;
    this._info = {
    ...
    while (i < chunk.length && this._phase !== ConnectionPhases.DONE) {
      switch (this._phase) {
        case ConnectionPhases.VERSION:
          if (readByte() !== SOCKS_VERSION) return closeSocketOnError();
        ...
        case ConnectionPhases.METHODS:
          {
            if (!this._authMethods) return closeSocketOnError();
            chunk.copy(this._authMethods, 0, i, i + chunk.length);
            if (!this._authMethods.includes(SOCKS_AUTH_METHOD.NO_AUTH)) return closeSocketOnError();
            ...
            i += minLen;
            if (this._methodsp !== this._authMethods.length) return closeSocketOnError();
            if (i < chunk.length) this._socket.unshift(chunk.slice(i));
        ...
        case ConnectionPhases.REQ_CMD:
          if (readByte() !== SOCKS_VERSION) return closeSocketOnError();
          const cmd = readByte();
          if (cmd !== SOCKS_CMD.CONNECT) return closeSocketOnError();
        ...
        case ConnectionPhases.REQ_ATYP:
          ...
          if (!(this._addressType in SOCKS_ATYP)) return closeSocketOnError();
          if (this._addressType === SOCKS_ATYP.IPv4) this._dstAddr = Buffer.alloc(4);else if (this._addressType === SOCKS_ATYP.IPv6) this._dstAddr = Buffer.alloc(16);else if (this._addressType === SOCKS_ATYP.NAME) this._phase = ConnectionPhases.REQ_DSTADDR_VARLEN;
          break;

        case ConnectionPhases.REQ_DSTADDR:
          {
            if (!this._dstAddr) return closeSocketOnError();
            ...
            i += minLen;
            if (this._dstAddrp === this._dstAddr.length) this._phase = ConnectionPhases.REQ_DSTPORT;
            break;
          }
  ...
  async ready() {
    ...
    return {
      info: this._info,
      forward: () => {
        const dstSocket = new _net.default.Socket();
...
class SocksInterceptedSocketHandler extends _instrumentation.SdkObject {
...
function writeSocksSocketError(socket, error) {
  ...
  socket.end(['HTTP/1.1 502 OK', 'Connection: close', 'Content-Type: text/plain', 'Content-Length: ' + Buffer.byteLength(body), '', body].join('\r\n'));
}

class SocksProxyServer {
  constructor(incomingMessageHandler) {
    this.server = void 0;
    this.server = _net.default.createServer(this._handleConnection.bind(this, incomingMessageHandler));
  }
  ...
  async _handleConnection(incomingMessageHandler, socket) {
    ...
    try {
      ...
      _debugLogger.debugLogger.log('proxy', `Could not parse: ${error} ${error === null || error === void 0 ? void 0 : error.stack}`);
