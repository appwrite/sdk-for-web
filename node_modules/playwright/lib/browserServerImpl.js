...
class BrowserServerLauncherImpl {
  ...
  async launchServer(options = {}) {
    ...
    if (options._acceptForwardedPorts) await playwright._enablePortForwarding(); // 1. Pre-launch the browser
    ...
    }, toProtocolLogger(options.logger)); // 2. Start the server
  ...
  async _onConnect(playwright, browser, scope, forceDisconnect) {
    ...
    new _playwrightDispatcher.PlaywrightDispatcher(scope, playwright, selectorsDispatcher, browserDispatcher);
    return () => {
