...
async function installDependenciesWindows(targets) {
  if (targets.has('chromium')) {
    const {
      code
    } = await utils.spawnAsync('powershell.exe', [_path.default.join(BIN_DIRECTORY, 'install_media_pack.ps1')], {
      cwd: BIN_DIRECTORY,
      stdio: 'inherit'
    });
...
async function installDependenciesLinux(targets) {
  ...
  const child = isRoot ? _child_process.default.spawn('sh', ['-c', `${commands.join('; ')}`], {
    stdio: 'inherit'
  }) : _child_process.default.spawn('sudo', ['--', 'sh', '-c', `${commands.join('; ')}`], {
    stdio: 'inherit'
  });
