...
class WKBrowser extends _browser.Browser {
  ...
  _onPageProxyCreated(event) {
    ...
    if (!context) return;
    const pageProxySession = new _wkConnection.WKSession(this._connection, pageProxyId, `The page has been closed.`, message => {
...
class WKBrowserContext extends _browserContext.BrowserContext {
  ...
  async _doAddInitScript(source) {
    ...
    for (const page of this.pages()) await page._delegate._updateBootstrapScript('main');
  }
