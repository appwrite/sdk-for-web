...
class WebServer {
  ...
  async _waitForAvailability() {
    ...
    const {
      timedOut
    } = await Promise.race([(0, _util.raceAgainstDeadline)(waitForSocket(this.config.port, 100, cancellationToken), launchTimeout + (0, _util.monotonicTime)()), this._processExitedPromise]);
