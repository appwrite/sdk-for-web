...
class WKPage {
  ...
  async _initializeSessionMayThrow(session, resourceTreeHandler) {
    ...
    for (const world of ['main', 'utility']) {
      ...
      if (bootstrapScript.length) promises.push(session.send('Page.setBootstrapScript', {
        ...
        worldName: webkitWorldName(world)
      }));
  ...
  _onDidCommitProvisionalTarget(event) {
    ...
    this._session.errorText = _javascript.kSwappedOutErrorMessage;
  ...
  async _onTargetCreated(event) {
    ...
    const session = new _wkConnection.WKSession(this._pageProxySession.connection, targetInfo.targetId, `The ${targetInfo.type} has been closed.`, message => {
  ...
  async _updateBootstrapScript(world) {
    await this._updateState('Page.setBootstrapScript', {
      source: this._calculateBootstrapScript(world),
      worldName: webkitWorldName(world)
    });
  }
  ...
  _handleRequestRedirect(request, responsePayload, timestamp) {
    ...
    response._requestFinished(responsePayload.timing ? _helper.helper.secondsToRoundishMillis(timestamp - request._timestamp) : -1, 'Response body is unavailable for redirect responses');
  ...
  _onResponseIntercepted(session, event) {
    ...
    if (!(route !== null && route !== void 0 && route._responseInterceptedCallback)) {
  ...
  _onResponseReceived(event) {
    ...
    if (event.response.requestHeaders && Object.keys(event.response.requestHeaders).length) request.request.updateWithRawHeaders((0, _utils.headersObjectToArray)(event.response.requestHeaders));
  ...
  _onLoadingFinished(event) {
    ...
    if (response) {
      var _event$metrics, _event$metrics2, _event$metrics2$secur, _responseReceivedPayl, _responseReceivedPayl2, _responseReceivedPayl3, _responseReceivedPayl4, _responseReceivedPayl5, _responseReceivedPayl6, _event$metrics3;
      ...
      response._requestFinished(_helper.helper.secondsToRoundishMillis(event.timestamp - request._timestamp), undefined, transferSize);
    }
