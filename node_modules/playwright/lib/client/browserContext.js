...
class BrowserContext extends _channelOwner.ChannelOwner {
  ...
  constructor(parent, type, guid, initializer) {
    ...
    this._channel.on('requestFinished', ({
      request,
      responseEndTiming,
      page
    }) => this._onRequestFinished(network.Request.from(request), responseEndTiming, _page.Page.fromNullable(page)));
  ...
  _onRoute(route, request) {
    for (const {
      ...
      if ((0, _clientHelper.urlMatches)(this._options.baseURL, request.url(), url)) {
