...
let path='/avatars/qr';return http.get(path,{'Content-type':'application/json'},{'text':text,'size':size,'margin':margin,'download':download})}};let database={listDocuments:function(collectionId,filters=[],offset=0,limit=50,orderField='$uid',orderType='ASC',orderCast='string',search='',first=0,last=0){if(collectionId===undefined){throw new Error('Missing required parameter: "collectionId"')}
...
let path='/storage/files';return http.post(path,{'Content-type':'application/json'},{'files':files,'read':read,'write':write,'folderId':folderId})},getFile:function(id){if(id===undefined){throw new Error('Missing required parameter: "id"')}
let path='/storage/files/{id}'.replace(new RegExp('{id}','g'),id);return http.get(path,{'Content-type':'application/json'},{})},deleteFile:function(id){if(id===undefined){throw new Error('Missing required parameter: "id"')}
let path='/storage/files/{id}'.replace(new RegExp('{id}','g'),id);return http.delete(path,{'Content-type':'application/json'},{})},getFileDownload:function(id){if(id===undefined){throw new Error('Missing required parameter: "id"')}
let path='/storage/files/{id}/download'.replace(new RegExp('{id}','g'),id);return http.get(path,{'Content-type':'application/json'},{})},getFilePreview:function(id,width=0,height=0,quality=100,background='',output=''){if(id===undefined){throw new Error('Missing required parameter: "id"')}
let path='/storage/files/{id}/preview'.replace(new RegExp('{id}','g'),id);return http.get(path,{'Content-type':'application/json'},{'width':width,'height':height,'quality':quality,'background':background,'output':output})},getFileView:function(id,as=''){if(id===undefined){throw new Error('Missing required parameter: "id"')}
let path='/storage/files/{id}/view'.replace(new RegExp('{id}','g'),id);return http.get(path,{'Content-type':'application/json'},{'as':as})}};let teams={listTeams:function(search='',limit=25,offset=0,orderType='ASC'){let path='/teams';return http.get(path,{'Content-type':'application/json'},{'search':search,'limit':limit,'offset':offset,'orderType':orderType})},createTeam:function(name,roles=["owner"]){if(name===undefined){throw new Error('Missing required parameter: "name"')}
...
if(password===undefined){throw new Error('Missing required parameter: "password"')}
let path='/users';return http.post(path,{'Content-type':'application/json'},{'email':email,'password':password,'name':name})},getUser:function(id){if(id===undefined){throw new Error('Missing required parameter: "id"')}
let path='/users/{id}'.replace(new RegExp('{id}','g'),id);return http.get(path,{'Content-type':'application/json'},{})},getUserLogs:function(id){if(id===undefined){throw new Error('Missing required parameter: "id"')}
let path='/users/{id}/logs'.replace(new RegExp('{id}','g'),id);return http.get(path,{'Content-type':'application/json'},{})},getUserPrefs:function(id){if(id===undefined){throw new Error('Missing required parameter: "id"')}
let path='/users/{id}/prefs'.replace(new RegExp('{id}','g'),id);return http.get(path,{'Content-type':'application/json'},{})},getUserSessions:function(id){if(id===undefined){throw new Error('Missing required parameter: "id"')}
let path='/users/{id}/sessions'.replace(new RegExp('{id}','g'),id);return http.get(path,{'Content-type':'application/json'},{})},deleteUserSessions:function(id){if(id===undefined){throw new Error('Missing required parameter: "id"')}
let path='/users/{id}/sessions'.replace(new RegExp('{id}','g'),id);return http.delete(path,{'Content-type':'application/json'},{})},deleteUsersSession:function(id,session){if(id===undefined){throw new Error('Missing required parameter: "id"')}
if(session===undefined){throw new Error('Missing required parameter: "session"')}
let path='/users/{id}/sessions/{session}'.replace(new RegExp('{id}','g'),id).replace(new RegExp('{session}','g'),session);return http.delete(path,{'Content-type':'application/json'},{})},updateUserStatus:function(id,status){if(id===undefined){throw new Error('Missing required parameter: "id"')}
if(status===undefined){throw new Error('Missing required parameter: "status"')}
let path='/users/{id}/status'.replace(new RegExp('{id}','g'),id);return http.patch(path,{'Content-type':'application/json'},{'status':status})}};let auth={register:function(email,password,name,redirect){return http.post(config.domain+'/'+config.version+'/users/auth/register',{'Content-type':'application/json'},JSON.stringify({'email':email,'password':password,'name':name,'redirect':redirect}))},login:function(email,password,success,failure){var form=document.createElement('form');form.setAttribute('method','post');form.setAttribute('action',config.domain+'/'+config.version+'/users/auth/login');var params={project:config.project,email:email,password:password,success:success,failure:failure};for(var key in params){if(params.hasOwnProperty(key)){var hiddenField=document.createElement("input");hiddenField.setAttribute("type","hidden");hiddenField.setAttribute("name",key);hiddenField.setAttribute("value",params[key]);form.appendChild(hiddenField)}}
document.body.appendChild(form);return form.submit()},};return{setEndpoint:setEndpoint,setProject:setProject,setKey:setKey,setLocale:setLocale,setMode:setMode,account:account,auth:auth,avatars:avatars,database:database,locale:locale,storage:storage,teams:teams,users:users}}})(window)