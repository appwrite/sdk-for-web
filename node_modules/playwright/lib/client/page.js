...
class Page extends _channelOwner.ChannelOwner {
  ...
  constructor(parent, type, guid, initializer) {
    ...
    this.keyboard = new _input.Keyboard(this._channel);
    this.mouse = new _input.Mouse(this._channel);
    this.touchscreen = new _input.Touchscreen(this._channel);
    ...
    this._channel.on('dialog', ({
      ...
      if (!this.emit(_events.Events.Page.Dialog, _dialog.Dialog.from(dialog))) dialog.dismiss().catch(() => {});
    });
  ...
  _onRoute(route, request) {
    for (const {
      ...
      if ((0, _clientHelper.urlMatches)(this._browserContext._options.baseURL, request.url(), url)) {
  ...
  _forceVideo() {
    if (!this._video) this._video = new _video.Video(this);
    return this._video;
  }
